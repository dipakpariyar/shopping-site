...
const updateUser = (getState, record) => getState.main.initialAdminData.ModelReview.map(data => data.id === record.id ? record : data); //eslint-disable-line

export default () => async (dispatch, getState) => {
  // console.log('add update modelReview called');
  const schema = 'modelReview';
  try {
    ...
    const newsRes = await axios.post(`${ENDPOINT}/web/add-update-model-review`, { ...record, token });
    ...
    if (newsRes.status === 200) {
      ...
      if (record.id) {
        dispatch(updateMainValue('initialAdminData', { ...getState().main.initialAdminData, ModelReview: updateUser(getState(), record) }));
      } else {
        dispatch(updateMainValue('initialAdminData', { ...getState().main.initialAdminData, ModelReview: [...getState().main.initialAdminData.ModelReview, { ...record, id: newsRes.data }] }));
      }
    }
  } catch (e) {
